...
3. Configure `crypttab`
    ...
    + If your encrypted volume contains the **root filesystem**, you might need to add this option to the **kernel command line** in your bootloader configuration using a format like: `rd.luks.options=tpm2-device=auto` 
        ...
        5. **Save and close** the `/etc/default/grub` file.
...
+ **Update:** I have written a simple CLI utility, `update-pcrs`, as a Bash script that simplifies and automates this process, and it saves you from having to memorize the commands and the appropriate arguments.
    + I have published this utility in its own Github repo, [here](https://github.com/benhaube/Update-LUKS-PCRs-script).
    + The `README.md` file in that repo has instructions showing you how to get the script installed.
    + The utility defaults to PCRs [0+4+7+11], but you can optionally supply custom PCRs in the command.
    + The utility will also work for your initial PCR enrollment, but you still need to manually edit the `/etc/crypttab` and `/etc/default/grub` files (if your **root** filesystem is encrypted) for the system to ask the TPM2 chip to unlock the encrypted volume. 
